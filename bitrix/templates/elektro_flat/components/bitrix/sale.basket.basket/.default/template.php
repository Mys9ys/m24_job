...
if(StrLen($arResult["ERROR_MESSAGE"])<=0):
	...
	<?$APPLICATION->IncludeComponent("altop:buy.one.click", "cart", 
		array(
			...
			"ELEMENT_ID" => "cart",
			"ELEMENT_PROPS" => "",
			"REQUIRED_ORDER_FIELDS" => array("NAME", "TEL"),
			...
			"BUY_MODE" => "ALL",
			"PRICE_ID" => "1",
			"DUPLICATE_LETTER_TO_EMAILS" => array("a"),
		),
		false
	);
...
/*if(!empty($arResult["ITEMS"]["AnDelCanBuy"])):
	$offersCatalogId = array();
	if(Loader::includeModule("catalog")) {
		$arSKU = CCatalogSKU::GetInfoByProductIBlock($arParams['IBLOCK_ID']);
		$offersCatalogId = (!empty($arSKU) ? $arSKU['IBLOCK_ID'] : 0);
	}

	if(!empty($offersCatalogId)):
		if(ModuleManager::isModuleInstalled("sale") && (!isset($arParams['USE_BIG_DATA']) || $arParams['USE_BIG_DATA'] != 'N')):?>
			<?$APPLICATION->IncludeComponent("bitrix:catalog.bigdata.products", ".default",
				array(
					...
					"OFFER_TREE_PROPS_".$offersCatalogId => $arParams["OFFERS_PROPERTY_CODE"],
					"ADDITIONAL_PICT_PROP_".$offersCatalogId => ""
				),
				false,
				array("HIDE_ICONS" => "Y")
			);?>
