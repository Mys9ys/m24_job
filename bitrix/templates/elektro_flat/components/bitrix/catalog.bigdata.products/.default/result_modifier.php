...
foreach($arResult["ITEMS"] as $key => $arElement):
	...
	/***PREVIEW_IMG***/
        if(is_array($arElement["PREVIEW_PICTURE"])) {
                ...
                $arElement['PREVIEW_IMG'] = $arResult["ITEMS"][$key]["PREVIEW_IMG"];
        }
        ...
        $tmpRes = CIBlockElement::GetList(
            ...
            false,
            false,
            Array('ID', 'IBLOCK_ID', 'PROPERTY_IMAGE_HOVER')
        );
        if($tmpItem = $tmpRes->Fetch())
        {
            if(!empty($tmpItem['PROPERTY_IMAGE_HOVER_VALUE'])) 
            {
                    $arFileTmp = CFile::ResizeImageGet(
                            $tmpItem['PROPERTY_IMAGE_HOVER_VALUE'],
                            array("width" => 300, "height" => 300),
                            BX_RESIZE_IMAGE_PROPORTIONAL,
                            true
                    );
