...
//Ссылки в head для поисковиков (КВ)
$currPage = $arResult["NavPageNomer"];
$lastPage = $arResult["NavPageCount"];

if($currPage > 1) 
{
	$APPLICATION->AddHeadString('<link rel="prev" href="http://'.$_SERVER['HTTP_HOST'].$APPLICATION->GetCurPage().'?PAGEN_1='.($currPage-1).'">');
}
if($currPage < $lastPage)
{
	$APPLICATION->AddHeadString('<link rel="next" href="http://'.$_SERVER['HTTP_HOST'].$APPLICATION->GetCurPage().'?PAGEN_1='.($currPage+1).'">');
}
//-----
