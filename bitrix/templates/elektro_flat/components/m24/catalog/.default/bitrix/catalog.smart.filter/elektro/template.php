	...
	<form name="<?=$arResult["FILTER_NAME"]."_form"?>" action="<?=$arResult["FORM_ACTION"]?>" method="get">
		...
		<table>
								...
								if(in_array($arItem["CODE"], $nsd)):
									foreach($arItem['VALUES'] as $val => $arOption):?>
										<div class="custom-forms">
											...
											<label data-role="label_<?=$arOption['CONTROL_ID']?>" <?=$arOption["DISABLED"] ? "class=\"disabled\"" : ""?> for="<?=$arOption['CONTROL_ID']?>"><?=$arItem["NAME"]?></label>
										</div>
									<?endforeach;
								endif;
							endforeach;?>
			...
			foreach($arResult["ITEMS"] as $key => $arItem):
				...
				if(($arItem["DISPLAY_TYPE"] == "A" || $arItem["DISPLAY_TYPE"] == "B") && (!$arItem["VALUES"]["MIN"]["VALUE"] || !$arItem["VALUES"]["MAX"]["VALUE"] || $arItem["VALUES"]["MIN"]["VALUE"] == $arItem["VALUES"]["MAX"]["VALUE"]))
					continue;
								...
								case "K": //RADIO_BUTTONS ?>
									...
									<?foreach($arItem["VALUES"] as $val => $arOption):?>
										<div class="custom-forms">
											...
											<label data-role="label_<?=$arOption['CONTROL_ID']?>" <?=$arOption["DISABLED"] ? "class=\"disabled\"" : ""?> for="<?=$arOption['CONTROL_ID']?>">
												<?=$arOption["VALUE"]?>
											</label>
										</div>
									<?endforeach;
		...
		<div class="clr"></div>
		<div class="submit">
			<a href="javascript:void(0)" class="catalog_item_toogle_filter_hidden"><?=GetMessage("FILTER_HIDDEN")?></a>
			<button type="submit" name="set_filter" class="btn_buy popdef" value="y"><?=GetMessage("FILTER_SET")?></button>
...
<div class="count_items">
	...
	<span><?=$arResult["ELEMENT_COUNT_NOT_AJAX"]?></span>
</div>
