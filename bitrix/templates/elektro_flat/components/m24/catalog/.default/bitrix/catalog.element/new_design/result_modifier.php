...
if(is_array($arResult["MORE_PHOTO"]) && count($arResult["MORE_PHOTO"]) > 0) {
	...
	foreach($arResult["MORE_PHOTO"] as $key => $arFile) {
        ...
        /***IMAGE_TOOLTIP***/
        if(!empty($arResult["PROPERTIES"]["image_tooltip"]))
        {
            if(is_array($arResult["PROPERTIES"]["image_tooltip"]))
            {
                $tmpImg = CFile::GetById($arResult["PROPERTIES"]["image_tooltip"]["VALUE"])->Fetch();
                $origSrc = '/upload/'.$tmpImg['SUBDIR'].'/'.$tmpImg['FILE_NAME'];
                $arFileTmp = CFile::ResizeImageGet(
                        $tmpImg,
                        array("width" => 220, "height" => 168),
                        BX_RESIZE_IMAGE_PROPORTIONAL,
                        true
                );
            }
            if(empty($arFileTmp["src"])) $arFileTmp["src"] = $origSrc;
            $arResult["IMAGE_TOOLTIP"] = array(
                    "SRC" => $arFileTmp["src"],
                    "WIDTH" => $arFileTmp["width"],
                    "HEIGHT" => $arFileTmp["height"],
            );
