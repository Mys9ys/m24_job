...
// Определяем значение заголовка Last-Modified для текущего раздела
GLOBAL $lastModified;
if (!$lastModified) 
{
	$res = CIBlockElement::GetList(
		Array("TIMESTAMP_X" => "DESC"),
		Array("IBLOCK_ID" => 39, "SECTION_ID" => $arResult["ID"]),
		false,
		Array("nTopCount" => 1),
		Array("TIMESTAMP_X")
	);
	$mod = $res->Fetch();
	$lastModified = max(MakeTimeStamp($mod["TIMESTAMP_X"]), MakeTimeStamp($arResult["TIMESTAMP_X"]));
}
