				...
				<div class="catalog-item-info">							
					...
					<div class="buy_more">
						<?if($arParams["DISPLAY_COMPARE"]=="Y"):?>
							<div class="compare">
								<a href="javascript:void(0)" data-url="<?=$arElement["COMPARE_URL"]?>" data-link="catalog_add2compare_link_<?=$arElement["ID"]?>" class="catalog-item-compare btn btn-default" id="catalog_add2compare_link_<?=$arElement['ID']?>" title="<?=GetMessage('CATALOG_ELEMENT_ADD_TO_COMPARE')?>" rel="nofollow">Сравнить</a>
							</div>
                                                        <script>
                                                            $(document).ready(function(){
                                                                $('#catalog_add2compare_link_<?=$arElement["ID"]?>').click(function(){
                                                                    addToCompare($(this).data('url'), $(this).data('link'));
                                                                });
                                                            })
                                                        </script>
						<?endif;?>
                                                <div class="view-detail">
                                                        <a href="<?=$arElement['DETAIL_PAGE_URL']?>" class="btn btn-default"><i class="fa fa-eye"></i></a>
                                                </div>				
					</div>
				</div>
			</div>
