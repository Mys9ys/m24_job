...
if (!empty($arResult['ITEMS']))
{
	...
	foreach ($arResult['ITEMS'] as $key => $arItem)
	{
                ...
                if(is_array($arItem["PREVIEW_PICTURE"])) {
                        ...
                        $arItem['PREVIEW_IMG'] = $arResult["ITEMS"][$key]["PREVIEW_IMG"];
                }
                ...
                if(!empty($arItem['PROPERTY_IMAGE_HOVER_VALUE'])) {
                        $arFileTmp = CFile::ResizeImageGet(
                                $arItem['PROPERTY_IMAGE_HOVER_VALUE'],
                                array("width" => 300, "height" => 300),
                                BX_RESIZE_IMAGE_PROPORTIONAL,
                                true
                        );

                        $arResult["ITEMS"][$key]["HOVER_IMG"] = array(
                                "SRC" => $arFileTmp["src"],
                                'WIDTH' => $arFileTmp["width"],
                                'HEIGHT' => $arFileTmp["height"],
                        );
                        $arItem['HOVER_IMG'] = $arResult["ITEMS"][$key]["HOVER_IMG"];
                }
                
                /***MANUFACTURER***/
                // тест2
                if($arItem["PROPERTIES"]["MANUFACTURER"]["VALUE"]):
                        $obElement = CIBlockElement::GetByID($arItem["PROPERTIES"]["MANUFACTURER"]["VALUE"]);
                        if($arEl = $obElement->GetNext()):
                                ...
                                $arItem["PROPERTIES"]["MANUFACTURER"]["NAME"] = $arResult["ITEMS"][$key]["PROPERTIES"]["MANUFACTURER"]["NAME"];
                                $arItem["PROPERTIES"]["MANUFACTURER"]["PREVIEW_IMG"] = $arResult["ITEMS"][$key]["PROPERTIES"]["MANUFACTURER"]["PREVIEW_IMG"];
                        endif;
                endif;
