...
while ($arItems = $dbBasketItems->Fetch()) {
    if ('' != $arItems['PRODUCT_PROVIDER_CLASS'] || '' != $arItems["CALLBACK_FUNC"]) {
        ...
        $arResult['BASKET_ITEMS'][$arItems['PRODUCT_ID']] = $arItems;
        $arResult['arBasketRule'][$arItems['PRODUCT_ID']] = $arItems['PRODUCT_ID'];
    }
}
...
{
    $arBasketRule[$arProductDiscounts['ID']]['items'][] = $arProductDiscounts['PRODUCT_ID'];
    $arBasketRule[$arProductDiscounts['ID']]['DISCOUNT_VALUE'] = intval($arProductDiscounts['VALUE']);
}
foreach ($arBasketRule as $arRule){
    $ar1count = count($arResult['arBasketRule']);
    $ar2count = count($arRule['items']);
    $result = array_diff($arResult['arBasketRule'], $arRule['items']);
    $resultCount = count($result);
    if(($ar1count-$ar2count) == $resultCount){
        $discountRuleCount += $arRule['DISCOUNT_VALUE'];
        ## убираем другие скидки на товары комплекта,если они есть
        foreach ($arResult['BASKET_ITEMS'] as $id=>$product){
            if($arResult['BASKET_ITEMS'][$id]['DISCOUNT_PRICE']>0){
                $arResult['summary_profit'] -= $arResult['BASKET_ITEMS'][$id]['DISCOUNT_PRICE'];
                $arResult['ORDER_PRICE']  += $arResult['BASKET_ITEMS'][$id]['DISCOUNT_PRICE'];
                $arResult['BASKET_ITEMS'][$id]['DISCOUNT_PRICE'] = 0;
            }
        }
    }
}
// вычитаем из общей суммы скидку комплекта и добавляем в сумму скидки
if($discountRuleCount > 0){
    $arResult['summary_profit'] +=$discountRuleCount;
    $arResult['ORDER_PRICE'] -= $discountRuleCount;
}
...
// Если есть хотя бы один товар в корзине с отключенным эквайрингом, то платежная система не показывается
    ...
    $arResult["BASKET_ITEMS"][$key]['SECTION_ID'] = $ar['IBLOCK_SECTION_ID'];
    $res = CIBlockSection::GetList(array("SORT" => "ASC"), array("IBLOCK_ID" => "39", 'ID' => $ar['IBLOCK_SECTION_ID']), false, $arSelect = array("NAME"));
    if($ar_res = $res->GetNext()){
        $result = $ar_res;
    }

    $arResult["BASKET_ITEMS"][$key]['SECTION_NAME'] = $result['NAME'];

    $res = CIBlockElement::GetList(array(), array("IBLOCK_ID" => "37",'ID' => $ar['PROPERTY_MANUFACTURER_VANUE']), false,false, $arSelect = array("NAME"));
    if($ar_res = $res->GetNext()){
        $vendor = $ar_res;
    }
    $arResult["BASKET_ITEMS"][$key]['MANUFACTURER_NAME'] = $vendor['NAME'];
//    dd($ar['IBLOCK_SECTION_ID']);
